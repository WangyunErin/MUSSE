...
private:
    ...
    int RandomPath();
    int maxOfRandom;
    int sortedArrayToBST(vector<Node*> nodes, int start, int end, int& pos, Bid& node);

public:
    ORAM *oram;
    int CommunicationSize;
    AVLTree(int maxSize, bytes<Key> key, Server* server, int userID);
    AVLTree(int maxSize, bytes<Key> key, MusesUserRunner* runner, int userID);
    AVLTree(int maxSize, bytes<Key> key, MusesOwnerRunner* runner, int userID);
    virtual ~AVLTree();
