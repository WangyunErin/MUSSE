...
AVLTree::AVLTree(int maxSize, bytes<Key> key, Server* server, int userID) {
    oram = new ORAM(maxSize, key, server, userID);
    maxOfRandom = (int) (pow(2, floor(log2(maxSize / Z)) + 1) - 1) / 2;
}

AVLTree::AVLTree(int maxSize, bytes<Key> key, MusesUserRunner* runner, int userID) {
    oram = new ORAM(maxSize, key, runner, userID);
    maxOfRandom = (int) (pow(2, floor(log2(maxSize / Z)) + 1) - 1) / 2;
}

AVLTree::AVLTree(int maxSize, bytes<Key> key, MusesOwnerRunner* runner, int userID) {
    oram = new ORAM(maxSize, key, runner, userID);
    maxOfRandom = (int) (pow(2, floor(log2(maxSize / Z)) + 1) - 1) / 2;
}
...
void AVLTree::finishOperation(bool find, Bid& rootKey, int& rootPos) {
    ...
    CommunicationSize = oram->CommunicationSize;
}
...
int AVLTree::sortedArrayToBST(vector<Node*> nodes, int start, int end, int& pos, Bid& node) {
    if (start > end) {
        pos = -1;
        node = 0;
        return 0;
